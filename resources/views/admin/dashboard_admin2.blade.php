                ...
                <table style="border-spacing: 0 15px;" class="table table-borderless table-hover" id="tabel-lapak"
                    ...
                    <tbody id="list-lapak">
                        <tr style="border-bottom: 1px solid #dcdde1;">
                            <td colspan="6" class="text-center fw-bold"> LAPAK MASIH KOSONG </td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
<script>
    $.ajaxSetup({
            headers: {
                ...
                'Authorization': "Bearer " + <?php echo json_encode($jwt_token) ?>
            }
    });

    function getLapak() {
        return $.ajax({
            url: "https://dev-ecommerce-api.paroki-gmaklaten.web.id/api/lapak/get", //url simpan data
            complete: function(response) {
                // console.log(response.responseJSON.data)
                let arrData = [];
                if (response.status == 200) {
                    $.each(response.responseJSON.data, function(index, data){
                        arrData.push(data);
                    });
    ...
    function getDetailLapak(data) {
        return $.ajax({
            url: "https://dev-ecommerce-api.paroki-gmaklaten.web.id/api/lapak/detail/" + data + "/get", //url simpan data
            complete: function(response) {
    ...
    $(document).ready(function() {
        ...
        async function printData(data) {
            let lapakAll = await getLapak();
            let lapakAllDetail = await Promise.all(lapakAll.data.map(data => getDetailLapak(data._id)))
            let lapakRender = [];
            lapakAllDetail.forEach(lapaks => {
            ...
            $("#list-lapak").remove();
            $("#tabel-lapak").append('<tbody id="list-lapak"></tbody>');
            for (let index = 0; index < lapakRender.length; index++) {
                const arrDate = lapakRender[index]['created_date'].split("-");
                const d = new Date(arrDate[2], --arrDate[1], arrDate[0]);
                lapakRender[index]['total_produk'] =lapakRender[index]['produk_lapak'].length;
                lapakRender[index]['time_created'] = d.getTime();
                lapakRender[index]['created_date'] = formatTanggal(d);
                $("#list-lapak").append(listLapak(lapakRender[index]));
            }
        };
